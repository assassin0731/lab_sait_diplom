...
<a href="#" align="center" style="font-size:18px;line-height:2.5; color:#4E4E4E; position:relative;left:1%; bottom: 8px;">Главная</a>
		<a align="center" style="font-size:18px;line-height:2.5; color:#909090; position:relative;left:1%; bottom:8px"> / </a>
			...
			<a align="center" style="font-size:18px;line-height:2.5; color:#909090; position:relative;left:1%; bottom:8px"> / </a>
				...
				<a align="center" style="font-size:18px;line-height:2.5; color:#909090; position:relative;left:1%; bottom:8px"> / </a>
					<a align="center" style="font-size:18px;line-height:2.5; color:#4E4E4E; position:relative;left:1%; bottom: 8px;">Акт</a>
  ...
  <div id="banner5" style=" height: 528px; width: 1235px; ">
	...
	<b>Выбор акта отбора</b>
		...
		<form action="/zakaz/zayavka/act/submit" method="post">
			...
			if ($zayav->name)
			{
				$fornew=DB::select('select id_zayav from otbors where id = ?',[$zayav->id_zayav]);
				foreach($fornew as $row){
				if ($row->id_zayav) {
			$fornew=$row->id_zayav;}}
			}
			else
			{
				$fornew=$zayav->id_zayav;
			}
			?>
...
<input name="id_zayav" id="id_zayav" autocomplete="id_zayav" value="{{$fornew}}" hidden>
	...
	<form action="/zakaz/zayavka/act/submit1" method="post">
		@csrf
...
<input name="id_zayav" id="id_zayav" autocomplete="id_zayav" value="{{$fornew}}" hidden>
	...
	<div  class="form-group row mb-0" style="position:absolute; left:37%; top:84px; color: #FFF !important;">
			<div  class="col-md-6 offset-md-4" >
					<button type="submit" id="buttonenter1" class="buttonenter1">
							{{ __('Акт отбора на смывы') }}
					</button>
			</div>
	</div>

</form>
<form action="/zakaz/zayavka/act/submit2" method="post">
	@csrf
...
<div  class="form-group row mb-0" style="position:absolute; left:57%; top:84px; color: #FFF !important;">
		<div  class="col-md-6 offset-md-4" >
				<button type="submit" id="buttonenter1" class="buttonenter1">
						{{ __('Акт отбора образцов') }}
				</button>
		</div>
</div>
