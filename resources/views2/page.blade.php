...
{{$day1[0]}}
  ...
  <p align="justify" style="font-size:16px;line-height: 1.5;">
	<?php $info=DB::select('select org from users,zayavkas where zayavkas.id = ? and users.id = zayavkas.id_user',[$city_name[5]]); foreach($info as $info1){echo $info1->org;} ?>
	<?php if (!$info) { $info=DB::select('select name from users,zayavkas where zayavkas.id = ? and users.id = zayavkas.id_user',[$city_name[5]]); foreach($info as $info1){echo $info1->name;} }?>
